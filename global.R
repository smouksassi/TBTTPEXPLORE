...
ttp_data <- read.csv("data/TTPDATA_Shiny_PRED.csv",
                     na.strings = c("", " ", ".", "NA", "na"),
...
makegompertzModelCurve<- function(tvOffset=4.33843,tvAlpha=22.4067,
  ...
  if (!TRT %in% c("RHZE","TMC207","Placebo (Background Regimen)","MICRONUTRIENT/RHZE","PA-824/PZ/M",
                  "PHZE","MRZE","User Sim")) {
    print(TRT)
    stop(paste("TRT=",TRT,"not supported, please choose a treatment from the following possibilities:
  ...
  BASELINEINFO<- data.frame(TRT=c("RHZE","TMC207","Placebo","MICRONUTRIENT/RHZE","PA-824/PZ/M","PHZE","MRZE"),
  ...
  if(TRT!="User Sim" ){
    ...
    REFBASELINETTP<- BASELINEINFO[BASELINEINFO$TRT==TRT,"REFBASELINETTP"]
  }
  if(TRT=="User Sim" ){
  ...
  Offset<-  tvOffset*(BASELINETTP/REFBASELINETTP)^dE0dBLMEANTTP* exp(dE0dTRT1*(TRT=="TMC207"))*
  ...
  Alpha <- tvAlpha*(BASELINETTP/REFBASELINETTP)^dAlphadBLMEANTTP* exp(dAlphadTRT1*(TRT=="TMC207"))*
    exp(dAlphadTRT2*(TRT=="Placebo")) * exp(dAlphadTRT5*(TRT=="MICRONUTRIENT/RHZE"))*
    exp(dAlphadTRT6*(TRT=="PA-824/PZ/M")) * exp(dAlphadTRT7*(TRT=="PHZE")) *
    exp(dAlphadTRT17*(TRT=="MRZE"))*
    ifelse(TRT=="User Sim",dAlphadTRTSIM,1)

  Beta <- tvBeta*(BASELINETTP/REFBASELINETTP)^dBetadBLMEANTTP* exp(dBetadTRT1*(TRT=="TMC207"))*
  ...
  Gamma<- tvGamma*(BASELINETTP/REFBASELINETTP)^dGamdBLMEANTTP* exp(dGamdTRT1*(TRT=="TMC207"))*
  ...
  TIMETTPMAX50 <-   log (  log ( (1/2 - Offset/(2*Alpha))^(- 1/Beta) )^( - 1/Gamma))
  TIMEEFFMAX50 <-   log (log( (0.5*(1+exp(-Beta)))^( - 1/Beta) )^(- 1/Gamma))
  ...
  df<- data.frame(Time=Time,TTPPRED=TTPPRED,
                  TIMETTPMAX50=TIMETTPMAX50,TIMEEFFMAX50=TIMEEFFMAX50,
                  TTPMAX50=TTPMAX50,EEFFMAX50=EEFFMAX50,
                  TTP0=TTP0,TTPINF=TTPINF,DeltaTTP=DeltaTTP,
                  TRT=TRT,
                  Offset=Offset,Alpha=Alpha,Beta=Beta,Gamma=Gamma,
                  BASELINETTP=BASELINETTP,REFBASELINETTP=REFBASELINETTP,
                  TTPAUC1=TTPAUC1,TTPAUC3=TTPAUC3,
                  OffsetTRT=OffsetTRT,AlphaTRT=AlphaTRT,BetaTRT=BetaTRT,GammaTRT=GammaTRT,
                  tvOffset=tvOffset,tvAlpha=tvAlpha,tvBeta=tvBeta,tvGamma=tvGamma,
                  OffsetBASELINEEFF=OffsetBASELINEEFF,AlphaBASELINEEFF=AlphaBASELINEEFF,BetaBASELINEEFF=BetaBASELINEEFF,
